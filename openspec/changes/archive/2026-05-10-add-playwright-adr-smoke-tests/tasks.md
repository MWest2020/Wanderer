...
- [x] 1.1 Where do screenshots live durably? Recommendation:
  `docs/screenshots/` for curated commits, `tests/playwright/screenshots/`
  gitignored for scratch.
- [x] 1.2 Run on every PR or nightly? Recommendation: every PR
  with binary cache; downgrade to a make-target if it ever
  exceeds 2 min.
- [x] 1.3 Node lockfile policy? Recommendation: ship
  `package-lock.json`, install with `npm ci --ignore-scripts`
  per the global CLAUDE.md rule.

## 2. Sign-off checkpoints

- [x] 2.1 Mark approves the scope (Chromium-only, no visual
  regression, screenshot-as-doc pipeline)
- [x] 2.2 Mark approves the ADR-coverage doc-lint check
- [x] 2.3 Mark approves the CI strategy

## 3. Pre-implementation inventory

- [x] 3.1 Walk every existing ADR in `docs/decisions/` and
  identify which carry UI claims — those need a Playwright
  spec
- [x] 3.2 List every archived OpenSpec change with UI scenarios
...
- [x] 4.1 When 1.x + 2.x are resolved, scaffold
