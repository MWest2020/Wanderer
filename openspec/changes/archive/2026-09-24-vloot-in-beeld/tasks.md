...
- [x] 1b.1 Top-3 toont een kale regel-ID bij regels zonder stroom
      (`wand.operationeel.caa_restricts_issuance`,
      `wand.accountability.ns_holder_transparent`), terwijl beide een
      handeling hebben.
- [x] 1b.2 Het aantal onbeantwoorde vragen staat nergens meer zichtbaar (alleen
      in het aria-label); de spec eist het apart.
- [x] 1b.3 De letters in het raster (S/V/N/?) hebben geen legenda.
- [x] 1b.4 Mobiel (390 px): de stroombalken zijn nul breed, en het raster laat
      de hele pagina horizontaal scrollen.

## 2. Uit
- [x] 2.1 `make playwright`, lint nul, screenshot vóór/na op een kopie van de
      prod-data.
- [x] 2.2 Release, deploy, nagekeken op de live instantie. (v0.11.0; live
      serveert de nieuwe CSS; de pagina zelf nagekeken met dezelfde binary op
      een kopie van de prod-data, want /ui/ zit achter Marks login.)
- [x] 2.3 ADR-0018 bijwerken (rendered surface); archiveren.
