# Delta for third-party-origin

> New capability — graduates from `research-high-signal-observability`
> (first Wave-2 lead, "what a surface pulls in"). Leads with the observed
> origin map; the existing `wand.technologie.third_parties_eea` rule
> annotates it. DICTU dimensions: Technologie (runtime dependency) and
> Juridisch (vendor jurisdiction). Richer cousin of the Wave-1 twins — a
> map of many vendors, not one endpoint.
