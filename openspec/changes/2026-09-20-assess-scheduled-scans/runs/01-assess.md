...
- [ ] 1.2 Faalt het beoordelen, dan blijft de scan staan, wordt de fout
  gelogd en gaat de volgende tick door.
- [ ] 1.3 `assess: false` per schedule; ontbreekt het veld, dan staat het
  aan. Let op: de configuratie wordt met `UnmarshalStrict` geparsed, dus
  bestaande bestanden moeten geldig blijven.
- [ ] 1.4 Tests: scan + assessment; `assess: false` → alleen scan;
  falende beoordeling → scan blijft staan.
- [ ] 2.1 Scheduling-documentatie + CHANGELOG onder `[Unreleased]`.
