...
- [x] 1.1 Migratie achteraan in `internal/store/migrations.go` plus
  ...
  op — alleen een hash.
- [x] 1.2 `POST /agents/enrol` in `internal/api`: neemt een token en een
  ...
  de drie het was.
- [x] 1.3 CLI in `cmd/wanderer`: een token aanmaken (met geldigheidsduur),
...
- [x] 1.4 Table-driven tests: token twee keer inwisselen faalt, verlopen
