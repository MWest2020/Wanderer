...
> (second Wave-2 lead). Leads with the observed front fact; the existing
> `wand.technologie.no_us_hyperscaler` rule annotates it. DICTU
> dimensions: Technologie (edge runtime dependency) and Juridisch (edge
> operator jurisdiction). Reframes the apex hosting picture for the
> CDN-fronted case.
